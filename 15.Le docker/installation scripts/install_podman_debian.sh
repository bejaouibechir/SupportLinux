#!/usr/bin/env bash
# ==============================================================================
# Installation complète de Podman (rootless) sur Debian 12/13 (bookworm/trixie)
...
if ! [ -r /etc/os-release ]; then
  echo "❌ Ce script est prévu pour Debian (12/13)." >&2
  exit 1
fi
. /etc/os-release
...
# --- [4] Vérification du service de socket ---------------------------------
echo "⚙️  Activation du service podman.socket..."
...
# Ajout des subuid/subgid pour rootless
...
echo "🎉 Podman rootless est installé et fonctionnel sur Debian."
